...
Run:
  python -m utility_propagation.run_grid --dataset adult
  python -m utility_propagation.run_grid --dataset bank

Outputs `utility_propagation/outputs/propagation_grid_results_{dataset}_<timestamp>.csv`.
Downstream: `python -m utility_propagation.fit_propagation --dataset adult|bank` (or `-i <csv>`).
"""
...
from utility_propagation.cleaners import build_cleaners
from utility_propagation.constants import RANDOM_STATE
from utility_propagation.dataset_profiles import PropagationDataset, get_profile
from utility_propagation.featurize import feature_signal_strength, prepare_xy, prepare_xy_columns
from utility_propagation.reference_and_dirty import (
    cleaning_quality_vs_reference,
    corrupt_categorical,
    load_tabular_csv,
    make_reference_and_test,
)
...
def run_one(
    ...
    if feat_columns is not None:
        X_tr, y_tr = prepare_xy_columns(cleaned, feat_columns, profile)
        X_te, y_te = prepare_xy_columns(ref_test, feat_columns, profile)
    elif feat_group in profile.feature_groups:
        X_tr, y_tr = prepare_xy(cleaned, feat_group, profile)
        X_te, y_te = prepare_xy(ref_test, feat_group, profile)
    elif feat_group in _WIDE_FEAT_GROUP_ALIASES:
        X_tr, y_tr = prepare_xy_columns(cleaned, None, profile)
        X_te, y_te = prepare_xy_columns(ref_test, None, profile)
    else:
        raise ValueError(
            f"feat_group {feat_group!r} is not in profile.feature_groups; pass feat_columns, "
...
def main(dataset: str = "adult", max_pool: int = 10000) -> None:
    profile = get_profile(dataset)
    cleaners = build_cleaners(list(profile.cat_error_cols))
    cleaner_names = list(cleaners.keys())
    feature_group_names = [k for k in profile.feature_groups.keys()]

    print(f"Loading {profile.name} from {profile.csv_path} ...", flush=True)
    df = load_tabular_csv(profile)
    ref_tr, ref_te, _, _ = make_reference_and_test(df, profile)
    if len(ref_tr) > max_pool:
        ref_tr = ref_tr.iloc[:max_pool].copy()
    cat_cols = [c for c in profile.cat_error_cols if c in ref_tr.columns]
    dirty_full, mask_full, cat_cols = corrupt_categorical(ref_tr, cat_cols, frac=0.22)
    ...
    print(f"Reference train + dirty copy ready ({len(ref_tr)} rows, shuffled).", flush=True)

    n_pool = len(ref_tr)
    fracs = list(profile.n_row_fracs) if profile.n_row_fracs is not None else N_ROWS_FRACS
    n_grid = [max(200, int(n_pool * f)) for f in fracs]
    ...
    print(
        f"Grid: dataset={profile.name}, train pool={n_pool} rows, n_rows in {n_grid}, "
    ...
    for n_rows, cname, fgroup, mname in itertools.product(
        ...
        if done == 1 or done % 40 == 0 or done == total:
            print(f"  progress: {done}/{total} configs", flush=True)

    out_df = pd.DataFrame(rows)
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    path = OUTPUT_DIR / f"propagation_grid_results_{profile.name}_{stamp}.csv"
    out_df.to_csv(path, index=False)
    print(f"Wrote {path} ({len(out_df)} rows)", flush=True)


if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Utility propagation grid over pipeline configs")
    ap.add_argument("--dataset", type=str, default="adult", help="adult | bank")
    ap.add_argument(
        "--max-pool",
        type=int,
        default=10000,
        help="Cap reference train rows (same default as Adult for comparable grid cost).",
    )
    args = ap.parse_args()
    main(dataset=args.dataset, max_pool=args.max_pool)
